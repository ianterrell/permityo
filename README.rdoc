...
To use a custom implementation, simply define +has_role?(role)+ on your user and +accepts_role?(role, user)+ on the relevant
models.

It is possible for another gem to provide alternate implementations for you, but none currently exist.

To change the settings, in the config block of your application, you can change the following settings from their defaults.

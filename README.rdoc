...
In your Gemfile:
  ...
  gem "constellation-authorization", :require => "constellation/authorization"
  
If you're using a Constellation Software gem that depends on this one, the above won't even be necessary.
...
All of the messages presented to the user are configurable and translatable.  To change the messages or provide a new translation, simply add the <tt>constellation.authorization.permission_denied</tt> and <tt>constellation.authorization.login_required</tt> keys to your locale file.  See <tt>config/locals/en.yml</tt> as an example.
...
You can override how the "redirection" is handled for each format by implementing the controller methods <tt>handle_login_required_redirection_for_#{format}</tt> and <tt>handle_permission_denied_redirection_for_#{format}</tt> for each given format, like so:
...
Copyright (c) 2010 Constellation Software for code version 2.0 and above.  Code originating in prior versions is copyright their respective authors.